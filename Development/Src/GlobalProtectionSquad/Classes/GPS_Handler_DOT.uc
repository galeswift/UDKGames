class GPS_Handler_DOT extends Actor;
	...
	if( HitsLeft > 0 )
	{
		...
		if (WeaponOwner.bSpreadEachHit && !WeaponOwner.bAOE)
		{
			if (WeaponOwner.NumSpreadTargets > NumSpreadTargets)
			{
				...
				if (NextTarget != none)
				{
					DOTHandler = Spawn(class'GPS_Handler_DOT', DOTTarget,,GPS_GameCrowdAgent(NextTarget).SkeletalMeshComponent.Bounds.Origin);
		...
		if (WeaponOwner.bAOE)
		{
			...
			foreach DynamicActors(class'GPS_GameCrowdAgent', A )
			{
				...
				{			
					...
					if (WeaponOwner.bSpreadEachHit)
					{				
						DOTHandler = Spawn(class'GPS_Handler_DOT', DOTTarget,,A.SkeletalMeshComponent.Bounds.Origin);
